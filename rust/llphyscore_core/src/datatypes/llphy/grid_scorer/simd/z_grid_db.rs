//! Module defining substructures of [`super::super::ZGridDB`]
//! using `#[portable_simd]`.
use crate::datatypes::llphy::grid_scorer::z_grid_db::{KNOWN_MAX_DATA_LEN, lookup_thorough};
use anyhow::Error;
    ...
    pub fn deserialize(buf: &mut &[u8], arena: &'a Bump) -> Result<Self, Error> {
        ...
        if data_len > KNOWN_MAX_DATA_LEN {
            return Err(Error::msg(format!(
                "[ZGridSubtable::deserialize] expected at most {} entries to be allocated, but asking for {}",
                KNOWN_MAX_DATA_LEN, data_len
            )));
        }
