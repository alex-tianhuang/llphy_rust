    ...
    pub fn deserialize(buf: &mut &[u8], arena: &'a Bump) -> Result<Self, Error> {
        ...
        if feature_names_len > KNOWN_FEATURE_NAMES_LEN {
            return Err(Error::msg(format!(
                ...
                KNOWN_FEATURE_NAMES_LEN, feature_names_len
            )));
        }
        let num_ref_seqs = <usize>::deserialize(buf)?;
        const KNOWN_NUM_REF_SEQS: usize = 20355;
        if num_ref_seqs > KNOWN_NUM_REF_SEQS {
            return Err(Error::msg(format!(
                "[ReferenceFeatureMatrix::deserialize] expected data for at most {} sequences to be allocated, but asking for {}",
                KNOWN_NUM_REF_SEQS, num_ref_seqs
            )));
        }
