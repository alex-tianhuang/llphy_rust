...
use llphyscore_core::datatypes::{
    AAMap, Aminoacid, AvgSdevDB, GridScorer, MAX_XMER, PairFreqDB, XmerSize, ZGridDB, ZGridDBEntry,
    ZGridSubtable,
};
use serde::Deserialize;
use serde_pickle::DeOptions;
use std::{collections::BTreeMap, fs::File, io::Read, mem::MaybeUninit, path::Path, ptr::addr_of_mut};
...
pub fn load_grid_scorer<'a>(scoredb_root: &Path, pair_name: &str, arena: &'a Bump) -> Result<&'a GridScorer<'a>, Error> {
    ...
    let subdir = scoredb_root.join(pair_name);
...
fn load_avg_sdev_db_into<'a>(
    ...
    for xmer in (1..=MAX_XMER).map(|x| XmerSize::new(x).unwrap()) {
